...
import { collectPages } from '../geo/geopdf/pageTree';
import { PdfDocument } from '../geo/geopdf/pdfReader';
import { isDict, isName, isStream } from '../geo/geopdf/types';
 ...
 *
 * One item is special: the large single-JPEG GeoPDF for `.maestro/pdf-overlays.yaml`
 * (#331) is generated by the same script but NOT checked in (6.6 MB), so its zip
 * is only verified when present — the e2e runner generates it first.
 */

const FIXTURE_DIR = join(__dirname, '..', '..', '..', '.maestro', 'fixtures', 'catalog');
const FIXTURE_INDEX_URL = 'http://127.0.0.1:8787/index.json';

/** The generated, gitignored Eco-class fixture (see scripts/catalog/make-fixture.ts). */
const LARGE_JPEG_ITEM_ID = 'fixture-jpeg-eco-class';
/** The iOS native crop path's decode budget the fixture must exceed to be a regression test. */
const NATIVE_DECODE_BUDGET_PIXELS = 16 * 1024 * 1024;
...
describe('checked-in e2e fixture catalog', () => {
  it('parses clean through the app index parser', () => {
    ...
    expect(index?.shards.length).toBeGreaterThanOrEqual(3);
    // The landing grid reads these, not the items — they must be right.
    expect(index?.categoryCounts).toEqual({ topo: 4, nautical: 2 });
  });
  ...
  it('puts both categories within reach of the emulator’s geo fix', () => {
    ...
    expect(new Set(picked.map((s) => s.category))).toEqual(new Set(['nautical', 'topo']));
    ...
    // The Eco-class fixture sits in Newfoundland precisely so it never enters
    // this section and store.yaml's five-row assertion stays true.
    expect(nearby.map((n) => n.item.id)).not.toContain(LARGE_JPEG_ITEM_ID);
  });
  ...
  it('ships a search digest that reaches items no nearby fetch would', () => {
    ...
    // …pdf-overlays.yaml types "jpeg" and must reach only the Eco-class fixture…
    expect(shardIdsForQuery(digest, 'jpeg')).toEqual(['topo-n40w060']);
  ...
  it('ships zips whose PDFs our importer can georeference', () => {
    for (const item of allItems()) {
      ...
      // The generated large fixture is gitignored; a checkout without it is fine.
      if (item.id === LARGE_JPEG_ITEM_ID && !existsSync(path)) continue;
  ...
  it('keeps the generated Eco-class fixture above the native decode budget (#331)', () => {
    const item = allItems().find((i) => i.id === LARGE_JPEG_ITEM_ID);
    expect(item).toBeDefined();
    const path = join(FIXTURE_DIR, item?.url.split('/').pop() ?? '');
    if (!existsSync(path)) return; // generated by scripts/catalog/make-fixture.ts, not checked in
    const pdf = extractPdf(new Uint8Array(readFileSync(path)));
    if (pdf === null) throw new Error('large fixture zip holds no PDF');
    const doc = PdfDocument.parse(pdf);
    const [page] = collectPages(doc);
    if (!page) throw new Error('large fixture has no page');
    // One page, one image XObject, DCTDecode, above the budget — the shape
    // that used to be refused into pdf.js and killed the WebView on iPhone.
    const resources = doc.resolve(page.dict.entries.get('Resources'));
    const xobjects = isDict(resources) ? doc.resolve(resources.entries.get('XObject')) : undefined;
    if (!isDict(xobjects)) throw new Error('large fixture page has no XObject resources');
    expect([...xobjects.entries.keys()]).toEqual(['Im1']);
    const image = doc.resolve(xobjects.entries.get('Im1'));
    if (!isStream(image)) throw new Error('large fixture image is not a stream');
    const { dict } = image;
    const filter = doc.resolve(dict.entries.get('Filter'));
    expect(isName(filter) && filter.name).toBe('DCTDecode');
    const width = Number(doc.resolve(dict.entries.get('Width')));
    const height = Number(doc.resolve(dict.entries.get('Height')));
    expect(width * height).toBeGreaterThan(NATIVE_DECODE_BUDGET_PIXELS);
  });
});
